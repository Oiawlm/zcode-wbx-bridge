...
界面风格自 v5.4 起为冻结契约：58 个设计 token、20 个组件、四页签信息架构与状态编码的**不变量**见 [UI-SPEC.md](UI-SPEC.md)——改契约须用户显式授权，文案与内容级增删不受限。
...
- 需要执行命令、写文件的任务（任何档位都不开；联网查资料与只读文件在 **L1 档**可外包，默认 L0 无工具——见上文「能力分级 caps」）
