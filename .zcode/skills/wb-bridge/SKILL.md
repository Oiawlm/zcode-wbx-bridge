...
description: >-
  ...
  批量生成/处理文本（翻译、摘要、改写、变体、结构化抽取、分类打标）、一段工作可拆成多个相互独立并行的子任务，
  或用户明确提到「并行处理、批量任务、外包出去、用免费额度、用 DeepSeek、WorkBuddy、CodeBuddy 跑」，或使用 /wbx 命令。
  前提：子任务纯文本进出、相互独立、单轮可完成。
---

# wb-bridge：把子任务并行分派给外部算力（WorkBuddy 双 lane · DeepSeek V4.1 Flash）

通过驱动 WorkBuddy 桌面版自带的 CodeBuddy CLI（headless 无工具模式），把适合的文本子任务
并行分派给账号下的 `deepseek-v4.1-flash`。双 lane：
...
**✅ 适合分派**（必须全部满足）：
- 相互独立，无顺序依赖，可并行
- 纯文本进出：输入能完整放进提示词，输出是文本或 JSON
- 单轮可完成，不需要工具、文件系统、联网、代码上下文
- 典型：多对象调研（每对象一个 worker，要求带来源清单）、中英互译、长文摘要、改写润色、
  标题/文案变体批量生成、资料整理草稿、结构化抽取（日志→JSON）、分类打标
...
指令全部写进用户消息、输出格式给硬约束（DeepSeek 官方建议）；声明无工具防幻觉：

```
你是一名{角色：如 资深中英翻译 / 产品调研分析师}。任务：{一句话任务}。
...
【输出要求】
- 只输出{明确的输出物：如"英文译文正文" / "一个 JSON 对象，字段 name、location" / "带来源清单的调研笔记"}，
  不要输出任何解释、前后缀、客套话或 Markdown 代码围栏
- 语言：{中文/英文}
- 不确定的内容用 "UNKNOWN" 占位，不要编造
```

要点：材料一次性贴全（模型看不到你的上下文）；单任务提示词控制在 2 万字符内，超长先拆分。
